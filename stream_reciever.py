...
def update_rolling_volume(raw_symbol, now):
    ...
    for map in symbol_mapper[raw_symbol]:
        ...
        average = baselines[parent_symbol, side, grouping, decay_bucket]
        mean_vol_35d = average["mean_vol_35d"]
        std_vol_35d = average["std_vol_35d"]
        mean_vol_3d = average["mean_vol_3d"]
        std_vol_3d = average["std_vol_3d"]
        z_vol_35d = (rolling_volume.get(raw_symbol, 0) - mean_vol_35d) / std_vol_35d
        z_vol_3d = (rolling_volume.get(raw_symbol, 0) - mean_vol_3d) / std_vol_3d
...
while True:
    ...
    if event["type"] == "quote_record":
        ...
        for map in metadata_rows:
            ...
            decay_bucket = map["decay_bucket"]
            average = baselines[parent_symbol, side, grouping, decay_bucket]
            mean_mid_35d = average["mean_mid_35d"]
            std_mid_35d = average["std_mid_35d"]
            mean_mid_3d = average["mean_mid_3d"]
            std_mid_3d = average["std_mid_3d"]
            z_mid_35d = (mid - mean_mid_35d) / std_mid_35d
            z_mid_3d = (mid - mean_mid_3d) / std_mid_3d
            mean_iv_35d = average["mean_iv_35d"]
            std_iv_35d = average["std_iv_35d"]
            mean_iv_3d = average["mean_iv_3d"]
            std_iv_3d = average["std_iv_3d"]
            current_iv = bs_iv_bisect(mid, underlying_price, strike, days_to_expiry, side)

            if z_mid_35d >= 1.5 and z_mid_3d >= 1.5:
                send_text(f"Mid alert for {raw_symbol}: parent = {parent_symbol} side = {side},grouping={grouping},z35={z_mid_35d}, z3={z_mid_3d}")
            if current_iv is not None:
                z_iv_35d = (current_iv - mean_iv_35d) / std_iv_35d
                z_iv_3d = (current_iv - mean_iv_3d) / std_iv_3d
                if z_iv_35d >= 1.5 and z_iv_3d >= 1.5:
